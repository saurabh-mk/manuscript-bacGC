...
with open(args_oi["wd"]+args_oi["list_fn"], "r") as required_genomes_details_fh:
    ...
    for line in required_genomes_details_fh.readlines():
        line_oi=line.strip()
        #make sure some id is available and it is not a MAG
        if((line_oi!="NA") & ("UBA" not in line_oi)):
            
            ##lets remove the initial letters
            ##they could be RS or GB or GCF or GCA
            ftp_folder_strings = line.strip().split("_")
            ##if acc num starts with RS or GB
            if(ftp_folder_strings[0]=="RS" or ftp_folder_strings[0]=="GB"):
                ##Create a string for required genome's folder at FTP           
                ftp_folder = ftp_folder_strings[1]+"/"+ftp_folder_strings[2][0:3]+"/"+ftp_folder_strings[2][3:6]+"/"+ftp_folder_strings[2][6:9]+"/"
            ##if it starts with GCF or GCA
            elif(ftp_folder_strings[0]=="GCF" or ftp_folder_strings[0]=="GCA"):
                ##Create a string for required genome's folder at FTP           
            ...
            ##create string for exact file required with extension
            ...
            ##if file is not found
                ...
                with open(args_oi["sd"]+args_oi["fn_type"]+"_GCFids_found.txt", "a") as f_fh:
                    f_fh.write("NA\n")
                print(e)
        else: #if acc num was not provided or it was a MAG
            print("not found")
            with open(args_oi["sd"]+args_oi["fn_type"]+"_GCFids_found.txt", "a") as f_fh:
                f_fh.write("NA\n")
