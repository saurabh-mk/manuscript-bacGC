---
title: "Various analysis related to GC jumps"
...
### Analysis of jump heights vs branch heights
Are jumps more frequent near the tips? Correlate cumulative distribution of branches and jumps.
```{r height_distributions }
all_edge_heights <- c()
jump_edge_heights <- c()
random_edge_heights <- c()

for(clade_n in 1:length(OLclades_oi)){
  OLclade_oi <- OLclades_oi[clade_n]
  ...
  detected_jump_edges <- which(tree_pp$edge.length>=pp_cutoff)
  heights_oi <- (nodeHeights(tree_in)[detected_jump_edges,1]+nodeHeights(tree_in)[detected_jump_edges,2])/(2*max(nodeHeights(tree_in)[,1]))
  jump_edge_heights <- c(jump_edge_heights, heights_oi)
  
  heights_oi <- (nodeHeights(tree_in)[,1]+nodeHeights(tree_in)[,2])/(2*max(nodeHeights(tree_in)[,1]))
  all_edge_heights <- c(all_edge_heights, heights_oi)
  
  random_sample <- sample(x = 1:length(tree_in$edge.length), size = 10*length(detected_jump_edges), prob =tree_in$edge.length )
  heights_oi <- (nodeHeights(tree_in)[random_sample,1]+nodeHeights(tree_in)[random_sample,2])/(2*max(nodeHeights(tree_in)))
  random_edge_heights <- c(random_edge_heights, heights_oi)
}

svg(filename = paste0(jump_common_dir, "height_distributions_branches_random_actual_jumps.svg"), width = 600, height = 900)
png(filename = paste0(jump_common_dir, "height_distributions_branches_random_actual_jumps.png"), width = 600, height = 900)
par(mfrow=c(3,1))
hist(all_edge_heights, breaks = 20, main=" (a) Height distribution of all branches", xlab="Normalized height above root", cex.main=2, cex.lab=1.5, cex.axis=1.5, xlim=c(0,1))
hist(random_edge_heights, breaks = 20, main="(b) Height distribution of branches w/ randomized jumps", xlab="Normalized height above root", cex.main=2, cex.lab=1.5, cex.axis=1.5, xlim=c(0,1))
abline(v = summary(random_edge_heights)[c(2,3,5)], col="orange", lty=2, lwd=2)
hist(jump_edge_heights, breaks = 20, main="(c) Height distribution of branches w/ actual jumps", xlab="Normalized height above root", cex.main=2, cex.lab=1.5, cex.axis=1.5, xlim=c(0,1))
abline(v = summary(jump_edge_heights)[c(2,3,5)], col="orange", lty=2, lwd=2)
dev.off()

wilcox.test(jump_edge_heights, random_edge_heights)
```