...
I have examined he precision-recall curves for simulated datasets. I find that for simulations on clades where the variance of jumps is similar to the variance of the Brownian process, then posterior probabilities (pps) > 0.75 are associated with precision >90%. For pps < 0.75, precision falls rapidly with pps. Therefore, I chose to use a cutoff of pp=0.75 in these cases (Sphingomonadale, Rhodobacterale, Rhizobiale, Alpha1, Flavobacteriale). However, for simulations on clades where the variance of jumps is lower than the variance of the Brownian process, then the precision falls rapidly with decreasing pps. Therefore, in this case, I chose to use a pp cutoff of 0.95 (Alpha1, Bacteroidale, Cytophagale). This also leads to >90% precision.
...
for(clade_n in 1:length(clades_oi)){
  ...
  svg(filename = paste0(out_dir,"levolution_",clade_oi,"_GTDB_14k_GC_config", config_oi, "_alpha", alpha_oi,"_pp", pp_cutoff, "_unlabelled_jumpNum_magma_scale.svg"), width = 25, height = 35)
  ...
  # svg(filename = paste0(out_dir,"levolution_",clade_oi,"_GTDB_14k_GC_config", config_oi, "_alpha", alpha_oi,"_pp", pp_cutoff, "_unlabelled_jumpNum_grey_scale.svg"), width = 25, height = 35)
